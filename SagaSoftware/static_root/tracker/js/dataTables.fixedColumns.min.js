...
var $jscomp = $jscomp || {};
$jscomp.scope = {};
$jscomp.getGlobal = function(a) { a = ["object" == typeof globalThis && globalThis, a, "object" == typeof window && window, "object" == typeof self && self, "object" == typeof global && global]; for (var h = 0; h < a.length; ++h) { var k = a[h]; if (k && k.Math == Math) return k } throw Error("Cannot find global object"); };
$jscomp.global = $jscomp.getGlobal(this);
$jscomp.checkEs6ConformanceViaProxy = function() {
    try {
        var a = {},
            h = Object.create(new $jscomp.global.Proxy(a, { get: function(k, e, d) { return k == a && "q" == e && d == h } }));
        return !0 === h.q
    } catch (k) { return !1 }
};
...
$jscomp.polyfill = function(a, h, k, e) { h && ($jscomp.ISOLATE_POLYFILLS ? $jscomp.polyfillIsolated(a, h, k, e) : $jscomp.polyfillUnisolated(a, h, k, e)) };
...
$jscomp.polyfillIsolated = function(a, h, k, e) {
    ...
    h = h(k);
    null != h && (a ? $jscomp.defineProperty($jscomp.polyfills, d, { configurable: !0, writable: !0, value: h }) : h !== k && ($jscomp.propertyToPolyfillSymbol[d] = $jscomp.IS_SYMBOL_NATIVE ? $jscomp.global.Symbol(d) : $jscomp.POLYFILL_PREFIX + d, d =
...
$jscomp.polyfill("Symbol", function(a) {
    ...
    var k = 0,
        e = function(d) { if (this instanceof e) throw new TypeError("Symbol is not a constructor"); return new h("jscomp_symbol_" + (d || "") + "_" + k++, d) };
    return e
}, "es6", "es3");
...
$jscomp.makeIterator = function(a) { var h = "undefined" != typeof Symbol && Symbol.iterator && a[Symbol.iterator]; return h ? h.call(a) : $jscomp.arrayIterator(a) };
...
$jscomp.polyfill("WeakMap", function(a) {
    ...
    function e(b) { var f = typeof b; return "object" === f && null !== b || "function" === f }
    ...
    if ($jscomp.USE_PROXY_FOR_ES6_CONFORMANCE_CHECKS) { if (a && $jscomp.ES6_CONFORMANCE) return a } else if (h()) return a;
    var m = "$jscomp_hidden_" + Math.random();
    g("freeze");
    g("preventExtensions");
    g("seal");
    var n = 0,
        c = function(b) { this.id_ = (n += Math.random() + 1).toString(); if (b) { b = $jscomp.makeIterator(b); for (var f; !(f = b.next()).done;) f = f.value, this.set(f[0], f[1]) } };
    ...
    c.prototype.get = function(b) { return e(b) && $jscomp.owns(b, m) ? b[m][this.id_] : void 0 };
    c.prototype.has = function(b) { return e(b) && $jscomp.owns(b, m) && $jscomp.owns(b[m], this.id_) };
    c.prototype.delete = function(b) { return e(b) && $jscomp.owns(b, m) && $jscomp.owns(b[m], this.id_) ? delete b[m][this.id_] : !1 };
    return c
}, "es6", "es3");
...
$jscomp.polyfill("Map", function(a) {
    ...
    var k = new WeakMap,
        e = function(c) {
            ...
            this.head_ = m();
            this.size = 0;
            if (c) { c = $jscomp.makeIterator(c); for (var b; !(b = c.next()).done;) b = b.value, this.set(b[0], b[1]) }
        };
    e.prototype.set = function(c, b) {
        c = 0 === c ? 0 : c;
        var f = d(this, c);
        f.list || (f.list = this.data_[f.id] = []);
        f.entry ? f.entry.value = b : (f.entry = { next: this.head_, previous: this.head_.previous, head: this.head_, key: c, value: b }, f.list.push(f.entry),
            this.head_.previous.next = f.entry, this.head_.previous = f.entry, this.size++);
        return this
    };
    e.prototype.delete = function(c) { c = d(this, c); return c.entry && c.list ? (c.list.splice(c.index, 1), c.list.length || delete this.data_[c.id], c.entry.previous.next = c.entry.next, c.entry.next.previous = c.entry.previous, c.entry.head = null, this.size--, !0) : !1 };
    e.prototype.clear = function() {
        this.data_ = {};
        this.head_ = this.head_.previous = m();
        this.size = 0
    };
    e.prototype.has = function(c) { return !!d(this, c).entry };
    e.prototype.get = function(c) {
        return (c =
            d(this, c).entry) && c.value
    };
    e.prototype.entries = function() { return g(this, function(c) { return [c.key, c.value] }) };
    e.prototype.keys = function() { return g(this, function(c) { return c.key }) };
    e.prototype.values = function() { return g(this, function(c) { return c.value }) };
    e.prototype.forEach = function(c, b) { for (var f = this.entries(), l; !(l = f.next()).done;) l = l.value, c.call(b, l[1], l[0], this) };
    ...
    var d = function(c, b) {
            ...
            if (l && $jscomp.owns(c.data_, f))
                for (c = 0; c < l.length; c++) { var u = l[c]; if (b !== b && u.key !== u.key || b === u.key) return { id: f, list: l, index: c, entry: u } }
        ...
        m = function() { var c = {}; return c.previous = c.next = c.head = c },
        n = 0;
    return e
}, "es6", "es3");
(function() {
    var a, h, k = function() {
        function e(d, g) {
            ...
            this.dom = { leftBottomBlocker: a("<div>").css(g).css("left", 0).addClass(this.classes.leftBottomBlocker), leftTopBlocker: a("<div>").css(g).css({ left: 0, top: 0 }).addClass(this.classes.leftTopBlocker), rightBottomBlocker: a("<div>").css(g).css("right", 0).addClass(this.classes.rightBottomBlocker), rightTopBlocker: a("<div>").css(g).css({ right: 0, top: 0 }).addClass(this.classes.rightTopBlocker) };
            ...
            d.on("column-sizing.dt", function() { return m._addStyles() });
            d.settings()[0]._fixedColumns = this;
            return this
        }
        e.prototype.left = function(d) { void 0 !== d && (this.c.left = d, this._addStyles()); return this.c.left };
        e.prototype.right = function(d) { void 0 !== d && (this.c.right = d, this._addStyles()); return this.c.right };
        ...
        e.prototype._getCellCSS = function(d, g, m) { return "left" === m ? this.s.rtl ? { position: "sticky", right: g + (d ? this.s.barWidth : 0) + "px" } : { left: g + "px", position: "sticky" } : this.s.rtl ? { left: g + "px", position: "sticky" } : { position: "sticky", right: g + (d ? this.s.barWidth : 0) + "px" } };
        ...
        e.prototype._setKeyTableListener = function() {
            ...
            this.s.dt.on("draw", function() { d._addStyles() });
            this.s.dt.on("column-reorder", function() { d._addStyles() });
            this.s.dt.on("column-visibility", function(g, m) { m.bDestroying || setTimeout(function() { d._addStyles() }, 50) })
        };
        e.version = "4.0.2";
        e.classes = {
    ...
    (function(e) {
        ...
        d("fixedColumns()", function() { return this });
        d("fixedColumns().left()", function(c) { var b = this.context[0]; return void 0 !== c ? (b._fixedColumns.left(c), this) : b._fixedColumns.left() });
        d("fixedColumns().right()",
            function(c) { var b = this.context[0]; return void 0 !== c ? (b._fixedColumns.right(c), this) : b._fixedColumns.right() });
        e.fn.dataTable.ext.buttons.fixedColumns = {
            action: function(c, b, f, l) { e(f).attr("active") ? (e(f).removeAttr("active").removeClass("active"), b.fixedColumns().left(0), b.fixedColumns().right(0)) : (e(f).attr("active", !0).addClass("active"), b.fixedColumns().left(l.config.left), b.fixedColumns().right(l.config.right)) },
